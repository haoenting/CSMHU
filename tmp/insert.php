...
//basic_information
  ...
  if ($stmt->affected_rows <= 0) {
    echo "資料1語法執行失敗，錯誤訊息: " . $stmt->error;
  }
  $stmt->close();
...
//PastHistory
  ...
  if ($stmt->affected_rows <= 0) {
    echo "資料2語法執行失敗，錯誤訊息: " . $stmt->error;
  }
   $stmt->close();
...
//Chemotherapy
  ...
  $insert_query = "INSERT INTO `action` VALUES ('$current_time', '$uid', 'insert $Chart')";
  mysqli_query($link, $insert_query);

  mysqli_close($link);
  echo "<script>alert('資料新增成功!'); window.location = 'insert.html';</script>";
?>